...
input_length = 600; % input 총 길이 ( 1~200, 401~600에선 0, 201~400까지 signal input)
...
signal = zeros(1, input_length); % signal 1~600까지 0으로 생성
signal(delay+1 : delay+N) = s; % delay=200이면 201~400번째 범위에 s(LFM signal) 적용

% noise only
noise = sqrt(noise_power/2) * (randn(1,input_length) + 1j*randn(1,input_length)); % AWGN 생성
